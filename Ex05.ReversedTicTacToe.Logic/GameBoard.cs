namespace Ex05.ReversedTicTacToe.Logic
{
    public class GameBoard
    {
        ...
        private readonly Cell<eSymbols>[,] r_BoardCells;
        ...
        public bool IsFoundSeries(eSymbols i_Symbol)
        {
            bool isFoundSeries = false;

            if (IsFullColumn(i_Symbol) || IsFullLeftDiagnoal(i_Symbol) || IsFullRightDiagnoal(i_Symbol) || IsFullRow(i_Symbol))
            {
                isFoundSeries = true;
            }
        ...
        {
            BoardCells[i_Row, i_Column].CurrentSymbol = default(eSymbols);
            BoardCells[i_Row, i_Column].IsAvailable = true;
        }

        public void SetCellToSymbol(int i_Row, int i_Column, eSymbols i_SymbolToInsert)
        {
            BoardCells[i_Row, i_Column].CurrentSymbol = i_SymbolToInsert;
            BoardCells[i_Row, i_Column].IsAvailable = false;
        }

        public bool CheckCellAvailability(int i_Row, int i_Col)
        {
            return BoardCells[i_Row, i_Col].IsAvailable;
        }
